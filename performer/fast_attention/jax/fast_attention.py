...
def nonnegative_softmax_kernel_feature_creator(data,
  ...
  data_scaled = data * data_normalizer
  ratio = 1.0 / jnp.sqrt(projection_matrix.shape[0])

  # Optimized: Use dot_general with broadcasting instead of thick matrix.
  data_dash = lax.dot_general(
      data_scaled,
      projection_matrix,
      (((data.ndim - 1,), (projection_matrix.ndim - 1,)),
       ((), ())),
      precision=precision)

  diag_data = jnp.sum(jnp.square(data_scaled), axis=data.ndim - 1, keepdims=True)
  diag_data = diag_data / 2.0

  last_dims_t = (len(data_dash.shape) - 1,)
  if is_query:
    max_axis = last_dims_t
  else:
    max_axis = last_dims_t + attention_dims_t

  data_dash = ratio * (
      jnp.exp(data_dash - diag_data -
              jnp.max(data_dash, axis=max_axis, keepdims=True)) + eps)
...
def generalized_kernel_feature_creator(data, projection_matrix, batch_dims_t,
  ...
  if projection_matrix is None:
    return kernel_fn(data_scaled) + kernel_epsilon
  else:
    # Optimized: Use dot_general with broadcasting instead of thick matrix.
    data_dash = lax.dot_general(
        data_scaled,
  ...
  def dot_product_attention(self,
    ...
    if self.unidirectional:
      ...
      if not self.renormalize_attention:
        ...
        # Optimized: Use sum instead of dot_general with all-ones tensor.
        # T = (K^{'})^{T} 1_L
