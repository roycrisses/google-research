  ...
  def dot_product_attention(self,
    ...
    if self.unidirectional:
      ...
      if not self.renormalize_attention:
        ...
        # Construct T = (K^{'})^{T} 1_L
        # key_prime (bs, <non-attention dims>, num_heads, <attention dims>, channels)
        # T (bs, <non-attention dims>, num_heads, channels)
        T = jnp.sum(key_prime, axis=attention_dims_t)

        # Construct partition function: R = Q^{'} T = Q^{'}(K^{'})^{T} 1_L
        # query_prime (bs, <non-attention dims>, num_heads, <attention dims>, channels)
        # T (bs, <non-attention dims>, num_heads, channels)
        # R (bs, <non-attention dims>, num_heads, <attention dims>)
        # We need to broadcast T over the <attention dims>.
        # query_prime is (batch_dims, attention_dims, channels)
        # T is (batch_dims, channels)
        # We want to multiply and sum over channels, keeping attention_dims.
        # This is equivalent to a dot product on the last dimension with broadcasting.
        R = jnp.sum(query_prime * jnp.expand_dims(T, axis=attention_dims_t), axis=-1)
