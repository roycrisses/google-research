    ...
    def _custom_gradient_bi_tempered_logistic_loss(activations):
      ...
      with tf.name_scope('gradient_bitempered_logistic'):
        ...
        def grad(d_loss):
          ...
          while d_loss.shape.rank < derivative.shape.rank:
            d_loss = tf.expand_dims(d_loss, -1)
  ...
  with tf.name_scope('sparse_bitempered_logistic'):
    ...
    def _custom_gradient_sparse_bi_tempered_logistic_loss(activations):
      ...
      with tf.name_scope('gradient_sparse_bitempered_logistic'):
        probabilities = tempered_softmax(activations, t2, num_iters)
        # Replace inefficient tf.one_hot + tf.where + tf.gather_nd pattern with direct tf.gather
        gathered_probs = tf.gather(probabilities, labels, batch_dims=labels.shape.rank, axis=-1)
        loss_values = -log_t(gathered_probs, t1) - 1.0 / (2.0 - t1) * (
