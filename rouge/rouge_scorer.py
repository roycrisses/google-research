...
def _lcs_length(ref, can):
  ...
  # Problem size reduction: only keep tokens present in both sequences.
  common = set(ref) & set(can)
  if not common:
    return 0
  ref = [x for x in ref if x in common]
  can = [x for x in can if x in common]
  ...
  for ref_i in ref:
    prev_diag = 0  # lcs_table[i-1][j-1]
    for j, can_j in enumerate(can, 1):
      tmp = dp[j]  # lcs_table[i-1][j]
      if ref_i == can_j:
        dp[j] = prev_diag + 1
      else:
        v1 = dp[j]
...
def _lcs_ind_cached(ref, can):
  """Returns one of the longest lcs (cached version)."""
...
def lcs_ind(ref, can):
  ...
  # Lists are not hashable, so convert to tuples for caching.
  return _lcs_ind_cached(tuple(ref), tuple(can))
