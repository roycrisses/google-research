...
def sinkhorn_iterations(x,
  ...
  def body_fn(f, g, eps, num_iter):
    for _ in range(inner_num_iter):
      inv_eps = 1.0 / eps
      # Pre-calculate cost_eps to avoid redundant divisions of the large cost matrix.
      cost_eps = cost * inv_eps
      if f.shape.rank == 2:
        # Optimized Sinkhorn update: use multiplication by inv_eps and
        # avoid redundant operations on cost.
        ...
        cost_eps_multi = cost[:, :, :, tf.newaxis] * inv_eps
        g = eps * (logb - tf.reduce_logsumexp(
            f[:, :, tf.newaxis, :] * inv_eps - cost_eps_multi, axis=1))
        f = eps * (loga - tf.reduce_logsumexp(
            g[:, tf.newaxis, :, :] * inv_eps - cost_eps_multi, axis=2))
