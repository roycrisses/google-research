...
def _log_marginal(cost, f, g, eps, axis):
  ...
  if axis == 1:  # marginal over the second dimension (b)
    if f.shape.rank == 2:
      return g_inv_eps + tf.reduce_logsumexp(
          f_inv_eps[:, :, tf.newaxis] - cost_eps, axis=1)
    return g_inv_eps + tf.reduce_logsumexp(
        f_inv_eps[:, :, tf.newaxis, :] - cost_eps[:, :, :, tf.newaxis], axis=1)

  if f.shape.rank == 2:
    return f_inv_eps + tf.reduce_logsumexp(
        g_inv_eps[:, tf.newaxis, :] - cost_eps, axis=2)
  return f_inv_eps + tf.reduce_logsumexp(
      g_inv_eps[:, tf.newaxis, :, :] - cost_eps[:, :, :, tf.newaxis], axis=2)
...
def cost_fn(x, y,
  ...
  if x.shape.rank == 2 and y.shape.rank == 2:
    ...
    if power == 1.0:
      ...
      # Use square instead of ** 2.0 for performance.
  ...
  # Otherwise data is high dimensional, in form [batch,n,d]. L2 distance used.
    ...
    # Note: this is missing a factor of 2 in front of the matmul to be true L2.
    # We preserve the existing behavior but optimize the power calculation.
    cost = x2[:, :, tf.newaxis] + y2[:, tf.newaxis, :] - tf.matmul(
        x, y, transpose_b=True)
    if power == 2.0:
      pass
    elif power == 1.0:
      cost = tf.math.sqrt(tf.math.maximum(cost, 0.0))
    else:
      cost = tf.math.maximum(cost, 0.0)**(power / 2.0)
    derivative = None
    return cost, derivative
...
def sinkhorn_iterations(x,
  ...
  cost_expanded = cost[:, :, :, tf.newaxis] if a.shape.rank > 2 else cost
  ...
  def body_fn(f, g, eps, num_iter):
    for _ in range(inner_num_iter):
      ...
      if f.shape.rank == 2:
        ...
        cost_eps_expanded = cost_expanded * inv_eps
        g = eps * (logb - tf.reduce_logsumexp(
            f[:, :, tf.newaxis, :] * inv_eps - cost_eps_expanded, axis=1))
        f = eps * (loga - tf.reduce_logsumexp(
            g[:, tf.newaxis, :, :] * inv_eps - cost_eps_expanded, axis=2))
