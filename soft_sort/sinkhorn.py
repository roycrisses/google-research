...
def error(cost, f, g, eps, b, logb=None):
  ...
  if logb is None:
    logb = tf.math.log(b)
  # We can rewrite |exp(log_b_target) - b| / b as |exp(log_b_target - log_b) - 1|.
  return tf.reduce_max(tf.abs(tf.exp(log_b_target - logb) - 1.0))
  ...
  def body_fn(f, g, eps, num_iter):
    for _ in range(inner_num_iter):
      ...
      if f_rank == 2:
        ...
        cost_eps_4d = cost[:, :, :, tf.newaxis] * inv_eps
        g = eps * (logb - tf.reduce_logsumexp(
            f[:, :, tf.newaxis, :] * inv_eps - cost_eps_4d, axis=1))
        f = eps * (loga - tf.reduce_logsumexp(
            g[:, tf.newaxis, :, :] * inv_eps - cost_eps_4d, axis=2))
